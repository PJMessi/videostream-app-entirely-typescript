 ...
 * @param rangeHeader
 */
export const determineStartAndEndBytes = (
  videoSize: number,
  rangeHeader: string
): [number, number] => {
 ...
 * @param rangeHeader
 */
const extratStartAndEndByteFromRangeHeader = (
  rangeHeader: string
): [number | undefined, number | undefined] => {
  const parsedRangeHeader = rangeHeader.replace(/bytes=/, '').split('-');

  const startByte =
    (parsedRangeHeader[0] && parseInt(parsedRangeHeader[0])) || undefined;
  const endByte =
    (parsedRangeHeader[1] && parseInt(parsedRangeHeader[1])) || undefined;
